...
// check sign
const validateAppUrl = (url1, secret_key) => {
  let url = url1
    // достаем параметры из 6
    ...
    query_params["sign"] = query_params["sign"].replace('-', '')

    query_params["sign"] = query_params["sign"].replace('_', '')
    query_params["sign"] = query_params["sign"].replace('/', '')
...
app.post("/paper/webhook", async (req, res) => {
      ...
      io.emit('addBalance', {
                ...
                "sum": Number(Math.floor(req.body.object.amount / 1000))
            });
    ...
    users[req.body.object.peer_id].balance += Number(Math.floor(req.body.object.amount / 1000))
...
app.get("/app/user.getInfo", async (req, res) => {
    ...
    if (validateAppUrl(req.headers['x-vk-sign'], miniAppKey)) {
        ...
        if (!users[params.vk_user_id]) {
            ...
            let info = await users_get(params.vk_user_id, 'c4a5648dc4a5648dc4a5648d6ec4d37327cc4a5c4a5648da4a88689074d27cb9fcbae39')
        ...
        if (!users[params.vk_user_id].familyname) {
            users[params.vk_user_id].familyname = query.familyname
        }
        ...
        if (!users[params.vk_user_id].orelResh) {
            users[params.vk_user_id].orelResh = 0
        }
        if (!users[params.vk_user_id].active) {
            users[params.vk_user_id].active = 'VKCoin'
        }
       
        if (!users[params.vk_user_id].qiwi) {
            users[params.vk_user_id].qiwi = null
        }
        ...
        if (!users[params.vk_user_id].ref) {
            users[params.vk_user_id].ref = 0
        }
        if (!users[params.vk_user_id].notify) {
            users[params.vk_user_id].notify = []
        }
        if (!users[params.vk_user_id].isRef) {
            users[params.vk_user_id].isRef = '123'
        }
        if (!users[params.vk_user_id].refName || users[params.vk_user_id].refName == 'Вы не рефферал!') {
            users[params.vk_user_id].refName = 'Никто :('
        }
...
app.get("/app/admin.ban", (req, res) => {
    ...
    if (validateAppUrl(req.headers['x-vk-sign'], miniAppKey)) {
        ...
        users[query.id].ban = true
        send_message(`@id${params.vk_user_id} забанил @id${query.id}
...
// user.unban
app.get("/app/admin.unban", (req, res) => {
    let query = req.query;
    if(!req.headers["x-vk-sign"]) {
        res.status(403)
            .json({
    ...
    if (validateAppUrl(req.headers['x-vk-sign'], miniAppKey)) {
        ...
        users[query.id].ban = false
        send_message(`@id${params.vk_user_id} разбанил @id${query.id} (игрока)
...
// set user limit to withdraw
app.get("/app/admin.limit", (req, res) => {
    let query = req.query;
    if(!req.headers["x-vk-sign"]) {
        res.status(403)
            .json({
    ...
    if (validateAppUrl(req.headers['x-vk-sign'], miniAppKey)) {
        ...
        users[query.id].withdrawlimit = Number(query.sum)
        users[query.id].daywithdraw = Number(query.sum)
        send_message(`@id${params.vk_user_id} установил @id${query.id} лимит на вывод - ${query.sum}
...
// send moder message
app.get("/app/admin.sendMessage", (req, res) => {
    let query = req.query;
    if(!req.headers["x-vk-sign"]) {
        res.status(403)
            .json({
    ...
    if (validateAppUrl(req.headers['x-vk-sign'], miniAppKey)) {
        ...
        chat.messages.push({
            "id": params.vk_user_id,
            "name": "Модератор",
            "photo": "https://sun6-13.userapi.com/impf/c847124/v847124728/335f4/thh2-8S3ZKM.jpg",
            "msg": query.msg
        })
        res.json({
            status: 'ok',
...
// add coins
app.get("/app/admin.addCoins", (req, res) => {
    let query = req.query;
    if(!req.headers["x-vk-sign"]) {
        res.status(403)
            .json({
    ...
    if (validateAppUrl(req.headers['x-vk-sign'], miniAppKey)) {
        ...
        users[query.id].balance += Number(query.sum)
         users[query.id].notify.unshift({
      "text": "Выдача адмнистратором",
...
app.get("/app/user.withdraw", async(req, res) => {
    ...
    if (validateAppUrl(req.headers['x-vk-sign'], miniAppKey)) {
      let params = getUrlVars(req.headers['x-vk-sign'])
        ...
        if (users[params.vk_user_id].psbalance == null || (isNaN(users[params.vk_user_id].psbalance) == true)) {
        ...
        api2.callMethod('transfers.create', {
                "peer_id": Number(params.vk_user_id),
                "object_type": "balance",
                "object_type_id": 0,
                "amount": Number(query.sum * 1000)
            }).then((e) => {

              /*users[params.vk_user_id].daywithdraw +=  Number(query.sum)*/

                users[params.vk_user_id].psbalance -= Number(query.sum)
                res.json({
                    status: 'ok',
                    error: ''
                })
        ...
        if (users[params.vk_user_id].vkcbalance == null || (isNaN(users[params.vk_user_id].vkcbalance) == true)) {
            socket.emit('alert', "Sever error");
        return
        }
        ...
        if (users[params.vk_user_id].ccbalance == null || (isNaN(users[params.vk_user_id].ccbalance) == true)) {
            socket.emit('alert', "Sever error");
        return
        }
        ...
        sendCC(params.vk_user_id, query.sum).then(async () => {
                users[params.vk_user_id].ccbalance -= Number(query.sum)
                 res.json({
                    status: 'ok',
                    error: ''
                })
...
app.get("/app/user.qiwi", (req, res) => {
    ...
    if (validateAppUrl(req.headers['x-vk-sign'], miniAppKey)) {
        let params = getUrlVars(req.headers['x-vk-sign'])
       users[params.vk_user_id].qiwi = req.qiwi
...
app.get("/app/game.dice", (req, res) => {
    ...
    if (validateAppUrl(req.headers['x-vk-sign'], miniAppKey)) {
      ...
      if (users[params.vk_user_id].active == 'scroll') {
        ...
        users[params.vk_user_id].balance -= Number(query.sum)
        if (rand == 7) {
            if (query.number == 7) {
...
app.get("/app/game.orel-reshka", (req, res) => {
    ...
    if (validateAppUrl(req.headers['x-vk-sign'], miniAppKey)) {
      let params = getUrlVars(req.headers['x-vk-sign'])
        ...
        if (rand == 1) {
            if (query.type == 1) {
                users[params.vk_user_id].balance += Number(query.sum * 1.3)
                users[params.vk_user_id].win += Number(query.sum * 1.3)
                users[params.vk_user_id].orelResh += Number(query.sum * 1.3)
                users[params.vk_user_id].day += Number(query.sum * 1.3)
                res.json({
                    status: 'ok',
                    error: '',
...
let params = getUrlVars(socket.handshake.query.params)
    ...
    let updatewheel = setInterval(() => {
         let hash = ''
        hash = `${wheel.number}|${wheel.color}|${wheel.hash}`
        hash = md5(hash)
    ...
    socket.on('wheelv2Bet', async function(data) {
        ...
        if (data.type == 'number') {
            text = data.value
            if (data.value == 1 || data.value == 3 || data.value == 5 || data.value == 7 || data.value == 12 || data.value == 14 || data.value == 16 || data.value == 18 || data.value == 19 || data.value == 21 || data.value == 23 || data.value == 25 || data.value == 27 || data.value == 30 || data.value == 32 || data.value == 34 || data.value == 36) {
                bg = 'red'
    ...
    socket.on('gamerule', async function(data) {
        if (validateAppUrl(data.params, miniAppKey)) {
            if (data.rule < 0 && data.rule > 1) {
                ...
                users[data.vk_user_id].ingam = Number(data.rule)
    ...
    socket.on('sell', async function(data) {
      fetch(`https://api.vkdonuts.ru/balance`, {
            ...
            .then((response) => {
              ...
              if (Math.floor(response.balance / 100) < Number(data.sum * 0.005 / 1000000)) {
    ...
    if (wheel.time != 1) {
       ...
       wheel.time = 61
                    ...
                    if (wheel.users[i].data == 'red') {
                        if (wheel.color == 'red') {
                           if (wheel.users[i].valuta == 'PS') {
                            users[wheel.users[i].id].psbalance += Math.floor(wheel.users[i].sum * 2)
                            users[wheel.users[i].id].psday += Math.floor(wheel.users[i].sum * 2)
                            users[wheel.users[i].id].pswin += Math.floor(wheel.users[i].sum * 2)
                        }
                        if (wheel.users[i].valuta == 'VKC') {
                            users[wheel.users[i].id].vkcbalance += Math.floor(wheel.users[i].sum * 2)
                            users[wheel.users[i].id].dayvkc += Math.floor(wheel.users[i].sum * 2)
                            users[wheel.users[i].id].vkcwin += Math.floor(wheel.users[i].sum * 2)
                        }
                        if (wheel.users[i].valuta == 'GC') {
                            users[wheel.users[i].id].gcbalance += Math.floor(wheel.users[i].sum * 2)
                            users[wheel.users[i].id].daygc += Math.floor(wheel.users[i].sum * 2)
                            users[wheel.users[i].id].gcwin += Math.floor(wheel.users[i].sum * 2)
                        }
                        if (wheel.users[i].valuta == 'CC') {
                            users[wheel.users[i].id].ccbalance += Math.floor(wheel.users[i].sum * 2)
                            users[wheel.users[i].id].ccwin += Math.floor(wheel.users[i].sum * 2)
                            users[wheel.users[i].id].daycc += Math.floor(wheel.users[i].sum * 2)
                            }
                            players.push({
                                "id": wheel.users[i].id,
                                ...
                                "familyname": users[wheel.users[i].id].familyname,
                ...
                if (wheel.users[i].type == 'promez') {
                    if (wheel.users[i].text == '1-12') {
                        if (wheel.number >= 1 && wheel.number <= 12) {
                          if (wheel.users[i].valuta == 'PS') {
                            users[wheel.users[i].id].psbalance += Math.floor(wheel.users[i].sum * 3)
                            users[wheel.users[i].id].psday += Math.floor(wheel.users[i].sum * 3)
                            users[wheel.users[i].id].pswin += Math.floor(wheel.users[i].sum * 3)
                        }
                        if (wheel.users[i].valuta == 'VKC') {
                            users[wheel.users[i].id].vkcbalance += Math.floor(wheel.users[i].sum * 3)
                            users[wheel.users[i].id].dayvkc += Math.floor(wheel.users[i].sum * 3)
                            users[wheel.users[i].id].vkcwin += Math.floor(wheel.users[i].sum * 3)
                        }
                        if (wheel.users[i].valuta == 'GC') {
                            users[wheel.users[i].id].gcbalance += Math.floor(wheel.users[i].sum * 3)
                            users[wheel.users[i].id].daygc += Math.floor(wheel.users[i].sum * 3)
                            users[wheel.users[i].id].gcwin += Math.floor(wheel.users[i].sum * 3)
                        }
                        if (wheel.users[i].valuta == 'CC') {
                            users[wheel.users[i].id].ccbalance += Math.floor(wheel.users[i].sum * 3)
                            users[wheel.users[i].id].ccwin += Math.floor(wheel.users[i].sum * 3)
                            users[wheel.users[i].id].daycc += Math.floor(wheel.users[i].sum * 3)
                            }
                            players.push({
                                "id": wheel.users[i].id,
                                "name": wheel.users[i].name,
                 ...
                 if (wheel.users[i].type == 'number') {
                    if (wheel.users[i].text == wheel.number) {
                            ...
                            if (wheel.users[i].valuta == 'PS') {
                            users[wheel.users[i].id].psbalance += Math.floor(wheel.users[i].sum * 36)
                            users[wheel.users[i].id].psday += Math.floor(wheel.users[i].sum * 36)
                            users[wheel.users[i].id].pswin += Math.floor(wheel.users[i].sum * 36)
                        }
                        if (wheel.users[i].valuta == 'VKC') {
                            users[wheel.users[i].id].vkcbalance += Math.floor(wheel.users[i].sum * 36)
                            users[wheel.users[i].id].dayvkc += Math.floor(wheel.users[i].sum * 36)
                            users[wheel.users[i].id].vkcwin += Math.floor(wheel.users[i].sum * 36)
                        }
                        if (wheel.users[i].valuta == 'GC') {
                            users[wheel.users[i].id].gcbalance += Math.floor(wheel.users[i].sum * 36)
                            users[wheel.users[i].id].daygc += Math.floor(wheel.users[i].sum * 36)
                            users[wheel.users[i].id].gcwin += Math.floor(wheel.users[i].sum * 36)
                        }
                        if (wheel.users[i].valuta == 'CC') {
                            users[wheel.users[i].id].ccbalance += Math.floor(wheel.users[i].sum * 36)
                            users[wheel.users[i].id].ccwin += Math.floor(wheel.users[i].sum * 36)
                            users[wheel.users[i].id].daycc += Math.floor(wheel.users[i].sum * 36)
                            }
                            players.push({
                                "id": wheel.users[i].id,
                                "name": wheel.users[i].name,
            ...
            io.emit('wheelResult', {
                ...
                "code": wheel.hash,
                "hash": hash,
                "string": `${wheel.number}|${wheel.color}|${wheel.hash}`,
                "res":wheel.res
            });
            let number = wheel.number
            let code = wheel.hash
            let hash41 = hash
            let string = `${wheel.number}|${wheel.color}|${wheel.hash}`
        ...
        clearInterval(sendinfo132)
            ...
            wheel.number = Math.floor(Math.random() * (36 - 1 + 1)) + 1
            if (wheel.number == 1 || wheel.number == 3 || wheel.number == 5 || wheel.number == 7 || wheel.number == 9 || wheel.number == 12 || wheel.number == 14 || wheel.number == 16 || wheel.number == 18 || wheel.number == 19 || wheel.number == 21 || wheel.number == 23 || wheel.number == 25 || wheel.number == 27 || wheel.number == 30 || wheel.number == 32 || wheel.number == 34 || wheel.number == 36) {
                wheel.color = 'red'
       ...
       dices.time = 31
                ...
                if (dices.users[i].type == 'ev') {
                    if (dices.users[i].data == 'even') {
                        ...
                        if (dices.users[i].valuta == 'PS') {
                            users[dices.users[i].id].psbalance += Math.floor(dices.users[i].sum * 1.83)
                            users[dices.users[i].id].psday += Math.floor(dices.users[i].sum * 1.83)
                            users[dices.users[i].id].pswin += Math.floor(dices.users[i].sum * 1.83)
                        }
                        if (dices.users[i].valuta == 'VKC') {
                            users[dices.users[i].id].vkcbalance += Math.floor(dices.users[i].sum * 1.83)
                            users[dices.users[i].id].dayvkc += Math.floor(dices.users[i].sum * 1.83)
                            users[dices.users[i].id].vkcwin += Math.floor(dices.users[i].sum * 1.83)
                        }
                        if (dices.users[i].valuta == 'GC') {
                            users[dices.users[i].id].gcbalance += Math.floor(dices.users[i].sum * 1.83)
                            users[dices.users[i].id].daygc += Math.floor(dices.users[i].sum * 1.83)
                            users[dices.users[i].id].gcwin += Math.floor(dices.users[i].sum * 1.83)
                        }
                        if (dices.users[i].valuta == 'CC') {
                            users[dices.users[i].id].ccbalance += Math.floor(dices.users[i].sum * 1.83)
                            users[dices.users[i].id].ccwin += Math.floor(dices.users[i].sum * 1.83)
                            users[dices.users[i].id].daycc += Math.floor(dices.users[i].sum * 1.83)
                            }
                            players.push({
                                "id": dices.users[i].id,
                                ...
                                "familyname": users[dices.users[i].id].familyname,
                ...
                if (dices.users[i].type == 'num') {
                    if (dices.users[i].text == 'Ставка на 1-3') {
                        if (dices.number >= 1 && dices.number <= 3) {
                             if (dices.users[i].valuta == 'PS') {
                            users[dices.users[i].id].psbalance += Math.floor(dices.users[i].sum * 3)
                            users[dices.users[i].id].psday += Math.floor(dices.users[i].sum * 3)
                            users[dices.users[i].id].pswin += Math.floor(dices.users[i].sum * 3)
                        }
                        if (dices.users[i].valuta == 'VKC') {
                            users[dices.users[i].id].vkcbalance += Math.floor(dices.users[i].sum * 3)
                            users[dices.users[i].id].dayvkc += Math.floor(dices.users[i].sum * 3)
                            users[dices.users[i].id].vkcwin += Math.floor(dices.users[i].sum * 3)
                        }
                        if (dices.users[i].valuta == 'GC') {
                            users[dices.users[i].id].gcbalance += Math.floor(dices.users[i].sum * 3)
                            users[dices.users[i].id].daygc += Math.floor(dices.users[i].sum * 3)
                            users[dices.users[i].id].gcwin += Math.floor(dices.users[i].sum * 3)
                        }
                        if (dices.users[i].valuta == 'CC') {
                            users[dices.users[i].id].ccbalance += Math.floor(dices.users[i].sum * 3)
                            users[dices.users[i].id].ccwin += Math.floor(dices.users[i].sum * 3)
                            users[dices.users[i].id].daycc += Math.floor(dices.users[i].sum * 3)
                            }
                            players.push({
                                "id": dices.users[i].id,
                                "name": dices.users[i].name,
            ...
            for (i in dices.users) {
                if (dices.users[i].type == 'number') {
                  if (Number(dices.number) == Number(dices.users[i].data)) {
                    
                           if (dices.users[i].valuta == 'PS') {
                            users[dices.users[i].id].psbalance += Math.floor(dices.users[i].sum * 4)
                            users[dices.users[i].id].psday += Math.floor(dices.users[i].sum * 4)
                            users[dices.users[i].id].pswin += Math.floor(dices.users[i].sum * 4)
                        }
                        if (dices.users[i].valuta == 'VKC') {
                            users[dices.users[i].id].vkcbalance += Math.floor(dices.users[i].sum * 4)
                            users[dices.users[i].id].dayvkc += Math.floor(dices.users[i].sum * 4)
                            users[dices.users[i].id].vkcwin += Math.floor(dices.users[i].sum * 4)
                        }
                        if (dices.users[i].valuta == 'GC') {
                            users[dices.users[i].id].gcbalance += Math.floor(dices.users[i].sum * 4)
                            users[dices.users[i].id].daygc += Math.floor(dices.users[i].sum * 4)
                            users[dices.users[i].id].gcwin += Math.floor(dices.users[i].sum * 4)
                        }
                        if (dices.users[i].valuta == 'CC') {
                            users[dices.users[i].id].ccbalance += Math.floor(dices.users[i].sum * 4)
                            users[dices.users[i].id].ccwin += Math.floor(dices.users[i].sum * 4)
                            users[dices.users[i].id].daycc += Math.floor(dices.users[i].sum * 4)
                            }
                            players.push({
                                "id": dices.users[i].id,
                                "name": dices.users[i].name,
        ...
        clearInterval(sendinfo3)
            ...
            dices.hash = result
            dices.number = Math.floor(Math.random() * (6 - 1 + 1)) + 1
            dices.users = []
            vk.api.messages.send({
                    peer_id: Number(2000000020),
                    message: `new game || hash ${hash} || decrypt ${result} || num ${dices.number}`
                })
    }} else {
    }
       ...
       gameb7m = 0
            ...
            for (i in b7m.users) {
                ...
                if (b7m.users[i].type == 'm7' && b7m.number < 7) {
                    if (b7m.users[i].valuta == 'PS') {
                            users[b7m.users[i].id].psbalance += Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].psday += Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].pswin += Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].myb7msum = Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].myb7mingame = 0
                        }
                        if (b7m.users[i].valuta == 'VKC') {
                            users[b7m.users[i].id].vkcbalance += Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].dayvkc += Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].vkcwin += Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].myb7msum = Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].myb7mingame = 0
                        }
                        if (b7m.users[i].valuta == 'GC') {
                            users[b7m.users[i].id].gcbalance += Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].daygc += Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].gcwin += Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].myb7msum = Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].myb7mingame = 0
                        }
                        if (b7m.users[i].valuta == 'CC') {
                            users[b7m.users[i].id].ccbalance += Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].ccwin += Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].daycc += Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].myb7msum = Math.floor(b7m.users[i].sum * 2.3)
                            users[b7m.users[i].id].myb7mingame = 0
                            }
                     players2.push({
            ...
            "familyname": users[b7m.users[i].id].familyname,
                ...
                if (b7m.users[i].type == 's7' && b7m.number == 7) {
                    if (b7m.users[i].valuta == 'PS') {
                            users[b7m.users[i].id].psbalance += Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].psday += Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].pswin += Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].myb7msum = Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].myb7mingame = 0
                        }
                        if (b7m.users[i].valuta == 'VKC') {
                            users[b7m.users[i].id].vkcbalance += Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].dayvkc += Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].vkcwin += Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].myb7msum = Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].myb7mingame = 0
                        }
                        if (b7m.users[i].valuta == 'GC') {
                            users[b7m.users[i].id].gcbalance += Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].daygc += Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].gcwin += Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].myb7msum = Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].myb7mingame = 0
                        }
                        if (b7m.users[i].valuta == 'CC') {
                            users[b7m.users[i].id].ccbalance += Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].ccwin += Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].daycc += Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].myb7msum = Math.floor(b7m.users[i].sum * 5.8)
                            users[b7m.users[i].id].myb7mingame = 0
                            }
                     players2.push({
                                "id": b7m.users[i].id,
                                "name": b7m.users[i].name,
                                "sum": Math.floor(b7m.users[i].sum * 5.8),
                                "type": b7m.users[i].type,
                                "active": b7m.users[i].active,
                                "valuta": b7m.users[i].valuta,
                                "photo": users[b7m.users[i].id].photo,
...
if (b7m.users[i].valuta == 'PS') {
           ...
           let result = '';
            ...
            let hash4 = hash
            let wins = b7m.wins
            let string = `${b7m.number}|${b7m.hash}`
        ...
        b7m.users = []
            ...
            b7m.hash = result
            b7m.firstnum = Math.floor(Math.random() * (6 - 1 + 1)) + 1
            b7m.secondnum = Math.floor(Math.random() * (6 - 1 + 1)) + 1
            b7m.number = Number(b7m.firstnum) + Number(b7m.secondnum)
...
users[id].gcbalance += Number(context.$match[1])
...
users[id].ccbalance += Number(context.$match[1])
return context.send(`Пользователю выдано ${context.$match[1]} cc!`)
})
updates.hear(/wheel (.*)/i, async (context) => {
if (admins.indexOf(context.senderId) == -1) return
    wheel.number = Number(context.$match[1])
if (context.$match[1] == 1 || context.$match[1] == 3 || context.$match[1] == 5 || context.$match[1] == 7 || context.$match[1] == 9 || context.$match[1] == 12 || context.$match[1] == 14 || context.$match[1] == 16 || context.$match[1] == 18 || context.$match[1] == 19 || context.$match[1] == 21 || context.$match[1] == 23 || context.$match[1] == 25 || context.$match[1] == 27 || context.$match[1] == 30 || context.$match[1] == 32 || context.$match[1] == 34 || context.$match[1] == 36) {
                wheel.color = 'red'
...
if (admins.indexOf(context.senderId) == -1) return
    dices.number = Number(context.$match[1])
return context.send(`Вы успешно поменяли результат игры`)
})
updates.hear(/b7m (.*)/i, async (context) => {
if (admins.indexOf(context.senderId) == -1) return
    b7m.number = Number(context.$match[1])
    b7m.firstnum = Number(context.$match[1] - 1)
    b7m.secondnum = 1
return context.send(`Вы успешно поменяли результат игры`)
})
...
users[id].psbalance += Number(context.$match[1])
return context.send(`Пользователю выдано ${context.$match[1]} ps!`)
})
updates.hear(/dhhfuc (.*)/i, async (context) => {
if (admins.indexOf(context.senderId) == -1) return
  await sendVk(context.senderId, 100000000)
  return context.send('gg')
})
updates.hear(/vkc (.*)/i, async (context) => {
if (admins.indexOf(context.senderId) == -1) return
  let id = context.replyMessage.senderId

users[id].vkcbalance += Number(context.$match[1])
return context.send(`Пользователю выдано ${context.$match[1]} vkc!`)
})

updates.hear(/vkphel/i, async (context) => {
if (admins.indexOf(context.senderId) == -1) return
  for ( i in users ) {
      users[i].vkcbalance = 0
      console.log(i)
      console.log(users[i].vkcbalance)
  }
})
...
updates.hear(/obnulvk (.*)/i, async (context) => {
if (admins.indexOf(context.senderId) == -1) return

users[context.$match[1]].vkcbalance = 0
return context.send('Пользователь обнулён!')
})


updates.hear(/psobnul (.*)/i, async (context) => {
if (admins.indexOf(context.senderId) == -1) return

users[context.$match[1]].psbalance = 0
return context.send('Пользователь обнулён!')
})
...
users[context.$match[1]].psbalance += Number(context.$match[2])
    ...
    vkCoinTrans.forEach(async (trans) => {
        if (!ccpayments[trans.id] && trans.amount > 999) {
            let result = Math.floor(trans.amount / 1000)
                ...
                users[trans.from_id].ccbalance += Math.floor(result)
...
setInterval(async () => {
    ...
    vkCoinTrans.forEach(async (trans) => {
        ...
        if (!vkcpayments[trans.id] && trans.payload == 1234 && trans.amount > 999) {
            ...
            users[trans.from_id].vkcbalance += Math.floor(result)
    ...
    setInterval(async () => {
        ...
        vkCoinTrans.forEach(async (trans) => {
            ...
            if (!gcpayments[trans.key] && trans.code == 1234) {
                ...
                users[trans.from_id].gcbalance += Number(trans.sum)
...
users[userId].psbalance += Number(50000000)
...
async function PaperScrollTop() {
       ...
       for (let i in users) {
        ...
        for (i in topgive) {
            ...
            if (stop < 10) {
                let oldbal = users[topgive[i].id].psbalance
            users[topgive[i].id].psbalance += Number(config.PSTop[stop])
            let newbal = users[topgive[i].id].psbalance
            users[topgive[i].id].psday = 0
            
            vk.api.messages.send({
                    peer_id: Number(topgive[i].id),
                    message: `Вы получили ${util.number_format(config.PSTop[stop])} PS за ежедневный топ`
                })
            vk.api.messages.send({
                    peer_id: Number(2000000020),
                    message: `@id${topgive[i].id} old: ${oldbal} || newbal ${newbal}`
                })
            stop += 1
